...
const UpdateStatus = () => {
  ...
  const updateStatus = async () => {
    setFieldError('');
    if (!user.trim()) {
      setFieldError('⚠️ Updated By field is required.');
      return;
    }
    try {
      await axios.patch(`http://localhost:8000/cases/${caseId}`, null, {
  ...
  return (
    <div className="form-container">
      <div className="case-form" style={{ maxWidth: '420px' }}>
        <h2 className="form-title">Search Case</h2>
        <div className="form-group full">
          <label htmlFor="caseId">Case ID<span style={{ color: 'red' }}> *</span></label>
          <input
            id="caseId"
            value={caseId}
            onChange={(e) => setCaseId(e.target.value)}
        ...
        <button type="button" className="submit-btn" onClick={fetchCase}>Fetch Case</button>
        {error && <p className="error-text">{error}</p>}
        {message && <p className="success-text">{message}</p>}
      </div>
      ...
      {caseData && (
        <div className="case-form" style={{ maxWidth: '750px' }}>
          ...
          <div className="form-grid">
            ...
            <div className="form-group full">
              <label htmlFor="status">New Status:<span style={{ color: 'red' }}> *</span></label>
              <select id="status" value={status} onChange={(e) => setStatus(e.target.value)}>
                {statusOptions.map((opt) => (
            ...
            <div className="form-group full">
              <label htmlFor="user">Updated By (User):<span style={{ color: 'red' }}> *</span></label>
              <input
                id="user"
                value={user}
                onChange={(e) => setUser(e.target.value)}
          ...
          <button type="button" className="submit-btn" onClick={updateStatus}>Update Status</button>
        </div>
      )}
    </div>
  );
