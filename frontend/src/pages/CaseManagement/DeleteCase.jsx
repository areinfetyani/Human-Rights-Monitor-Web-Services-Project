...
const DeleteCase = () => {
  ...
  const deleteCase = async () => {
    try {
      ...
      setMessage('');
      setError(err?.response?.data?.detail || '❌ Failed to delete/archive case.');
    }
  };
      ...
      {caseData && (
        <div className="case-form" style={{ maxWidth: '750px', flex: '1 1 600px' }}>
          ...
          <div className="form-grid">
            <div className="form-group full">
              <label>Title</label>
              <input value={caseData.title} disabled />
            </div>
            <div className="form-group full">
              <label>Description</label>
              <textarea rows="3" value={caseData.description} disabled />
            </div>
            <div className="form-group">
              <label>Status</label>
              <input value={caseData.status} disabled />
            </div>
            <div className="form-group">
              <label>Priority</label>
              <input value={caseData.priority} disabled />
            </div>
            <div className="form-group">
              <label>Country</label>
              <input value={caseData.location?.country || ''} disabled />
            </div>
            <div className="form-group">
              <label>Region</label>
              <input value={caseData.location?.region || ''} disabled />
            </div>
            <div className="form-group full">
              <label>Victims</label>
              <input value={caseData.victims?.join(', ') || ''} disabled />
            </div>
            <div className="form-group full">
              <label>Violation Types</label>
              <input value={caseData.violation_types?.join(', ') || ''} disabled />
            </div>
          </div>
